...
if (run_env$status %in% c("NEW", "RESUME")) {
  ...
  for (sp in all_species_temporal) {
    ...
    # Locate the canonical markdown from Module 10
    canonical_path <- file.path(run_env$run_dir, "canonical",
                                paste0(sp_clean, "_canonical.md"))
    if (!file.exists(canonical_path)) {
      log_warn("No canonical MD for %s, skipping temporal.", sp, module = "MAIN")
      next
    }
