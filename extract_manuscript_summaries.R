...
if (!is.null(delta_all) && nrow(delta_all) > 0) {
  ...
  # ── Narrative note, COMPUTED rather than asserted ────────────────────────
  # This paragraph used to be hardcoded to the v1.0 -> v1.1 transition, where no
  # presence records were added. Stated unconditionally it would be simply false
  # on any run that did add records -- the same failure mode as a supplement
  # describing an intended design rather than the built one.
  n_ext_total <- sum(delta_all$new_extinctions, na.rm = TRUE)
  gained <- delta_all[!is.na(delta_all$record_delta) & delta_all$record_delta > 0, , drop = FALSE]
  lost   <- delta_all[!is.na(delta_all$record_delta) & delta_all$record_delta < 0, , drop = FALSE]

  if (nrow(gained) == 0) {
    add("Note: **no new presence records** were added between v%s and v%s.", V_BASE, V_CURR)
    if (n_ext_total > 0) {
      add("Across all species/scopes the change is **%d distinct extirpated localities**",
          n_ext_total)
      add("(extinctions_count counts unique coordinates, not rows). Every record delta is")
      add("therefore negative: records leave the active set via the extinction flag plus the")
      add("500 m backward suppression mask. EOO is unchanged wherever the extirpated points")
      add("sat inside the convex hull, which is why AOO moves but EOO often does not.")
    }
  } else {
    add("Between v%s and v%s, **%d scope(s) across %d species gained records** and %d lost them.",
        V_BASE, V_CURR, nrow(gained), length(unique(gained$species)), nrow(lost))
    if (n_ext_total > 0)
      add("There are also **%d distinct newly extirpated localities** (extinctions_count", n_ext_total)
    if (n_ext_total > 0)
      add("counts unique coordinates, not rows).")
  }

  # ── New and expanding non-indigenous occurrence (Reviewer 2, line 72) ────
  # "In addition to highlighting extinctions, it would be valuable to also
  #  display new invasions (species introductions)."
  # The data was always computed; only extinctions were being surfaced.
  ni <- delta_all[delta_all$scope == "non-indigenous", , drop = FALSE]
  newly_intro <- ni[!is.na(ni$records_prev) & ni$records_prev == 0 &
                    !is.na(ni$records_curr) & ni$records_curr > 0, , drop = FALSE]
  expanding   <- ni[!is.na(ni$records_prev) & ni$records_prev > 0 &
                    !is.na(ni$record_delta) & ni$record_delta > 0, , drop = FALSE]

  add("")
  add("### 6b. New and expanding non-indigenous occurrence")
  add("")
  add("The counterpart to the extinction signal: species gaining ground rather than")
  add("losing it. Both are derived from the same per-species version comparison.")
  add("")
  add("| Signal | Species |")
  add("|---|---|")
  add("| Newly recorded as non-indigenous (0 -> >0 records) | %d |", nrow(newly_intro))
  add("| Established and expanding (non-indigenous records increased) | %d |", nrow(expanding))
  add("")
  if (nrow(newly_intro) > 0) {
    add("**Newly recorded as non-indigenous:**")
    add("")
    for (i in seq_len(nrow(newly_intro))) {
      r <- newly_intro[i, ]
      add("- *%s* — %s records, AOO %s km2", r$species, fm(r$records_curr), fm(r$AOO_curr))
    }
    add("")
  }
  if (nrow(expanding) > 0) {
    e <- expanding[order(-expanding$record_delta), , drop = FALSE]
    add("**Established and expanding** (largest gains first):")
    add("")
    for (i in seq_len(min(nrow(e), 20))) {
      r <- e[i, ]
      add("- *%s* — %s -> %s records (+%s), AOO %s -> %s km2",
          r$species, fm(r$records_prev), fm(r$records_curr), fm(r$record_delta),
          fm(r$AOO_prev), fm(r$AOO_curr))
    }
    if (nrow(e) > 20) add("- ... and %d more", nrow(e) - 20)
    add("")
  }
  if (nrow(newly_intro) == 0 && nrow(expanding) == 0)
    add("*No non-indigenous gains between these two versions.*")
