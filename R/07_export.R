    ...
    for (species_name in species_list) {
      ...
      if (!is.null(occurrences_df) && "species" %in% names(occurrences_df)) {
        ...
        if (nrow(sp_rows) > 0) {
          ...
          # Darwin Core: relabel the EXPOSED occurrence table to DwC-aligned
          # names (internal names/values are untouched upstream). Coordinates are
          # already stripped above, so no decimalLatitude/Longitude leak.
          sp_rows_safe <- dwc_rename_output_columns(sp_rows_safe)
