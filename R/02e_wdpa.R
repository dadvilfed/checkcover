  ...
  with_log_section(module, {
    ...
    .fetch_wdpa_one <- function(iso, target_crs) {
      ...
      if (file.exists(cp)) {
        ...
        x <- readRDS(cp)
        x <- sanitize_spatial_layer(x, layer_name = sprintf("WDPA_%s_cached", iso))
      } else {
      ...
      #}
        ...
        x <- sf::st_as_sf(x)
        x <- sf::st_zm(x, drop = TRUE, what = "ZM")
        x <- sanitize_spatial_layer(x,
                                    target_crs = target_crs,
                                    layer_name = sprintf("WDPA_%s_fresh", iso))
        .save_with_lock(x, cp)
        log_info("Cached WDPA for %s", iso, module = module)
      }
      # CRS handling now inside sanitize_spatial_layer
      .std_geom(x)
    }
    ...
    ne <- sanitize_spatial_layer(ne, layer_name = "NE_countries_WDPA")
