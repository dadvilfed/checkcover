...
.build_indigenous_narrative <- function(sp, reports, vern_map, feow_map, hb_names) {
  ...
  # Fragmentation
  frag <- report$spatial_clustering %||% report$fragmentation
  frag_text <- ""
  if (!is.null(frag) && frag$computed) {
    if (frag$status == "detected") {
      frag_text <- sprintf(
        "**Spatial clustering:** Detected. The species distribution shows %d spatial clusters (%s of all localities).",
        frag$n_clusters, frag$cluster_sizes_n
      )
    } else if (frag$status == "none_detected") {
      frag_text <- "**Spatial clustering:** None detected. All known localities form a single spatial cluster."
    }
  } else {
    frag_text <- "**Spatial clustering:** Not computed (insufficient data or not applicable for cosmopolitan ranges)."
  }
...
.build_non_indigenous_narrative <- function(sp, reports, vern_map, feow_map, hb_names) {
  ...
  full_text <- paste(
    ...
    "",
    "## Notes",
    "Spatial clustering analysis is not applicable for non-indigenous populations.",
    sep = "\n"
  )
