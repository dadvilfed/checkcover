...
calculate_indigenous_metrics <- function(result_indigenous, output_dir = "checkover_output") {
  ...
  with_log_section(module, {
    ...
    metrics <- cd_active %>%
      ...
      summarise(
        n_records = n(),
        # Excludes NA/blank AND `unresolved` — an unresolvable record must not
        # inflate the country count or influence the category.
        n_countries = n_distinct_geo(country),
        # Thresholded: blank/NA labels excluded, and a continent must hold >=5
        # records AND >=5% of this species' labelled records. Stops a single
