...
compute_species_fingerprint <- function(sp_data, columns = .FINGERPRINT_COLUMNS) {
  ...
  # Sort rows for row-order independence. Prefer record_id when it actually
  # distinguishes rows; but the DwC-aligned template omits occurrenceID (assigned
  # at upload), so record_id can be uniformly NA — in that case order() on it is
  # a no-op and the fingerprint would depend on input row order. Fall back to a
  # deterministic multi-column sort so the fingerprint is stable regardless of
  # how the rows happened to be ordered on disk. Legacy data with real record_ids
  # keeps its existing sort (and existing fingerprints) unchanged.
  if ("record_id" %in% names(df_canon) &&
      length(unique(df_canon$record_id)) > 1L) {
    df_canon <- df_canon[order(df_canon$record_id), , drop = FALSE]
  } else {
    df_canon <- df_canon[do.call(order, as.list(df_canon)), , drop = FALSE]
  }
...
# ---------------------------------------------------------------------------
# SHARED TREND SOURCE (Module 9 package_metadata + Module 10 narrative)
# ---------------------------------------------------------------------------
# One implementation so the narrated trend equals package_metadata.json's
# trend_vs_previous exactly (Lucian bug 5 — single source of truth).

#' Trend vs previous version from AOO change (±5% threshold).
#' Vocabulary matches package_metadata.json: "increase" / "decrease" / "stable",
#' or NA when either side is missing (population absent in current or prior).
compute_trend_vs_previous <- function(curr_aoo, prev_aoo) {
...
#' Prior-version AOO (indigenous + non-indigenous) for a species.
#'
#' Walks ctx$prior_versions (numeric-desc) for the most recent manifest entry
#' carrying the species, then reads that source version's package_metadata.json
#' for prior AOO. Returns is_baseline = TRUE when no prior entry exists. This is
#' the single lookup shared by Module 9 (metadata trend) and Module 10
#' (narrative trend), so both compute an identical trend.
read_prior_species_aux <- function(ctx, sp_clean) {
  empty <- list(is_baseline = TRUE, previous_version = NULL,
                prior_aoo_indigenous = NULL, prior_aoo_non_indigenous = NULL)
  if (is.null(ctx) || is.null(ctx$prior_versions)) return(empty)
