...
enrich_with_gadm <- function(result, output_dir = "checkover_output",
  ...
  with_log_section(module, {
    ...
    log_info("Countries detected in data (ISO3): %s",
             paste(iso_list, collapse = ", "), module = module)
    
    # Initialize output columns
    result$clean_data$country <- NA_character_
    result$clean_data$admin_1 <- NA_character_
    result$clean_sf$country <- NA_character_
    result$clean_sf$admin_1 <- NA_character_
    
    log_debug("Initialized 'country' and 'admin_1' columns.", module = module)
    
    # Precompute point indices per ISO3
    pts_idx_by_iso <- split(seq_len(nrow(result$clean_sf)), iso_per_point)
    ...
    for (iso_raw in iso_list) {
      ...
      gadm_sf <- sf::st_make_valid(gadm_sf)
      ...
      if (used_level == 1 && has_NAME_1) {
        ...
        if (any(na_mask)) {
          log_info("Filling %d missing NAME_1 entries using nearest GADM feature for %s.",
                   sum(na_mask), iso, module = module)
          nearest_idx <- sf::st_nearest_feature(joined[na_mask, ], gadm_sf)
          joined$NAME_1[na_mask] <- gadm_sf$NAME_1[nearest_idx]
          if (has_NAME_0 && "NAME_0" %in% names(gadm_sf)) {
            joined$NAME_0[na_mask] <- gadm_sf$NAME_0[nearest_idx]
          }
        }
      }
      ...
      # Write back
      result$clean_sf$country[idx] <- vec_country
      result$clean_sf$admin_1[idx] <- vec_admin1
      result$clean_data$country[idx] <- vec_country
      result$clean_data$admin_1[idx] <- vec_admin1
    }
