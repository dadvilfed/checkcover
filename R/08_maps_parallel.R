...
generate_all_maps_par <- function(scenario_table,
  ...
  with_log_section(module, {
    ...
    generate_species_maps <- function(sp) {
      ...
      # ═══ ACTIVE-SET FILTER (mirrors 08_maps.R) ═══
      # Draw EOO/AOO/basins from the same active set that drives the metrics, so
      # extirpated points never paint a range. A zero-active species yields an
      # empty slice and is skipped below. NOTE: return(NULL) is safe here because
      # this body is a real function (generate_species_maps) — unlike the
      # sequential module, where it had to be wrapped explicitly.
      if (!is.null(sp_data) && "temporal_status" %in% names(sp_data)) {
        sp_data <- sp_data[!is.na(sp_data$temporal_status) & sp_data$temporal_status == "active", , drop = FALSE]
      }
      if (!is.null(sp_sf) && "temporal_status" %in% names(sp_sf)) {
        sp_sf <- sp_sf[!is.na(sp_sf$temporal_status) & sp_sf$temporal_status == "active", , drop = FALSE]
      }
    ...
    # Summary. Guard the empty (0 active species) case — see 08_maps.R.
