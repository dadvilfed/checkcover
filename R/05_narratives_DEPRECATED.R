      ...
      if (!is.null(result$fragmentation)) {
        ...
        if (nrow(frag_row) > 0) {
          if (frag_row$scope == "not_applicable_for_cosmopolitan_ranges") {
            frag_text <- "Distributional fragmentation signal: not computed, as this species is classified as cosmopolitan."
          } else if (as.logical(frag_row$computed)) {
            if (frag_row$status == "none_detected") {
              frag_text <- "Distributional fragmentation signal: none detected (all known localities form a single spatial cluster under the mean-distance threshold)."
            } else if (frag_row$status == "detected") {
              frag_text <- glue::glue(
                "Distributional fragmentation signal: detected ({frag_row$n_clusters} spatial clusters identified under the mean-distance threshold; cluster sizes: {frag_row$cluster_sizes_n} of all localities)."
              )
            }
          } else {
            frag_text <- "Distributional fragmentation signal: not computed (insufficient data points)."
          }
        }
      }
