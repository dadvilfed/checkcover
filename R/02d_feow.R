...
load_feow_min <- function(cache_dir, feow_source = "auto", feow_shp_path = NULL,
  ...
  if (file.exists(cache_path)) {
    ...
    feow_min <- sanitize_spatial_layer(feow_min, layer_name = "FEOW_cached")
  ...
  if (feow_source == "feowR") {
    ...
    if (!inherits(feow_sf, "try-error")) {
      feow_sf <- sanitize_spatial_layer(feow_sf, layer_name = "FEOW_source")
    }
    ...
    if (is.null(feow_sf) && !is.null(feow_shp_path)) {
      ...
      feow_sf <- try(sf::st_read(feow_shp_path, quiet = TRUE), silent = TRUE)
      if (!inherits(feow_sf, "try-error")) {
        feow_sf <- sanitize_spatial_layer(feow_sf, layer_name = "FEOW_source_fallback")
      }
...
enrich_with_feow <- function(result, output_dir = "checkover_output",
  ...
  with_log_section(module, {
    ...
    # zone can match multiple FEOW polygons via st_within, returning more
    # rows than the input. De-duplicate by keeping the first match per
    # input row. The first match is the row that st_within ranked highest.
    if (nrow(joined) > nrow(result$clean_sf)) {
      log_warn("st_join produced %d rows from %d input points; deduplicating to first match per point.",
               nrow(joined), nrow(result$clean_sf), module = module)
      joined$.input_row_id <- seq_len(nrow(joined))
      # In sf::st_join the input rows are preserved in order, with duplicates
      # appearing as consecutive rows. The first match per input is the one
      # whose row index in `joined` matches the input row's position.
      # Easier: use record_id (or whatever the unique key is) to dedupe.
      key_col <- intersect(c("record_id", "id", "ID"), names(joined))[1]
      if (!is.na(key_col)) {
        joined <- joined[!duplicated(joined[[key_col]]), , drop = FALSE]
      } else {
        # Fallback: dedupe by geometry-identity
        joined <- joined[!duplicated(sf::st_as_text(sf::st_geometry(joined))), , drop = FALSE]
      }
      joined$.input_row_id <- NULL
      log_info("After dedup: %d rows.", nrow(joined), module = module)
    }
