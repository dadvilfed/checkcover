...
| `test_kml_determinism.R` | The same data gives the same KML: every KML written through a temp file names its layer (statically, everywhere), and on Linux a real write twice gives identical bytes with no random `file4d2a…` name inside (runs in the image build; skipped on Windows, where sf crashes under Rscript) | base R; `sf` on Linux |
| `test_robust_joins.R` | Point-in-polygon work survives rings the spherical engine refuses (a repeated vertex, as a crop to a narrow bounding box can leave): the robust helpers repair and retry and give the plain result on valid polygons; `enrich_with_feow()` completes for a narrow-bbox taxon; no spatial module joins directly any more. Written after the first single-taxon trial died in the FEOW join | `sf`, `dplyr` |
| `test_single_definitions.R` | Parses (never runs) every file `checkcover_main.R` sources, in its order: no function is defined twice (the later copy would silently win), and every named argument in a call reaches a definition that accepts it. Written after a stale `resolve_basin_names()` in 04_reports.R killed the first full 1.0 run | base R only |
